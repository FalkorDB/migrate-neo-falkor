...
def get_neo_credentials():
    ...
    user = (
        input(f"Enter Neo4j username (default: {NEO_CREDS_USERNAME}): ").strip()
        or NEO_CREDS_USERNAME
    )
    password = (
        input(f"Enter Neo4j password (default: {NEO_CREDS_PASSWORD}): ").strip()
        or NEO_CREDS_PASSWORD
    )
